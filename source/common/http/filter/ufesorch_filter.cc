...
void UfesOrchFilter::complete(OrchestratorResponse& response) {
	...
	ENVOY_LOG(debug, "Orchestrator request error with response code {}.", response.getResponseCode());
   	...
   	if (!response.getCacheKey().empty()) {
   		downstream_headers_->addStaticKey(CacheConstants::get().CacheKeyHeader, response.getCacheKey());
   		downstream_headers_->addStaticKey(CacheConstants::get().GoToOrigin, response.getGoToOrigin());
   	}
   }
	// continue decoding.
