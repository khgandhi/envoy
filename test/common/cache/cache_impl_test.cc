...
static Http::TestHeaderMapImpl genRequestHeadersToOrchestratorClient() {
	 return Http::TestHeaderMapImpl{{"test", "test"}, {":authority", "test.example.com"}, {":path", "/test/path"}};
}
	...
	EXPECT_CALL(cm.async_client_, send_(_, _, _))
	  .WillOnce(
		  Invoke([&](Http::MessagePtr&, Http::AsyncClient::Callbacks& callbacks,
			...
			callbacks.onSuccess(Http::MessagePtr {new Http::ResponseMessageImpl(Http::HeaderMapPtr{new Http::TestHeaderMapImpl{{"cache-key", "1234"}, {":status", "200"}}})});
			return nullptr;
		  }));
